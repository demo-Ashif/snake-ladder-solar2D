...
showAwarenesMessage = 2; --2 means message will be shown 
...
local function btnTap1(event)
	--p1.x = bg.x+4*bgw+5*bgsh;--same position as dont score 1 for 1st palyer 
	--p1.y = bg.y+5*bgh+10*bgsh;--same position as dont score 1 
	--p2.x = bg.x+3*bgw+5*bgsh;--for same position as dont score 1 for 2nd palyer 
	--p2.y = bg.y+5*bgh+10*bgsh;--same position
...
local function btnTap3()
	storyboard.gotoScene( "snakeMenu" )
				--starting new everything
	return true
end
...
function scene:createScene(event)
   ...
   --reloadbutton
	...
	------- Second Player
    p2 = display.newImage("images/snake/a.png"); 
	...
	p2.y = display.contentHeight-60;
	group:insert(p2)
	p22 = display.newImage("images/snake/a.png"); 
	--boo:setReferencePoint(display.BottomReferencePoint);
		  ...
		  --print(rand);
			...
			elseif playstart[i]==0 then
				...
				if(playerCount%2==1) then
					...
					timer.performWithDelay( 750, timerE )
				end
			end
		  --print(i..playstart[i]);
				...
				if diceshot~=nil then
					...
					if  snake==1  or lad==1 then 
							if snake==1 then
								...
								-- Showing awarenes Message warning 
								awarenesMessageWarn()
							end	
							if lad==1 then
								local  last1 = audio.play( ladders) ;
								-- Showing awarenes Message good things 
								awarenesMessageGood()
							end
						if i==1 then	
			...
			if playstart[i]==0 then
				if i==1 then
					p1.x = bg.x-1*bgw+5*bgsh;
					p1.y = display.contentHeight-60;
					player.text="Android turn!  ".. 100-diceshot[2].." to win"
            ...
            elseif i==1 then
				    ...
				    if animation1 ~=nil then
				       ...
				       tapenable = true
						...
						player.text="Android turn!  ".. 100-diceshot[2].." to win"--for head line
					if animation1~=nil	then
						animation1:removeSelf()
						animation1 = display.newSprite( mySheet1, sequenceData1 )
						animation1.x = bg.x-2*bgw+22*bgsh
		    ...
		    elseif i==2 then
					...
					ai = false
					if rand==1 then
						player.text="Android turn!  ".. 100-diceshot[2].." to win"--after score 1 for rolling again
			...
			if diceshot[i]==100 then
				...
				win=1
				if i==1 then
					player.text="player"..i.."  is  winner";
				else 
					player.text="Android is winner"
				end
				if i==1 then
						local alert = native.showAlert( "Player  "..i.."  is  winner","congratulation", { "Ok",  }, onComplete )
				else
				  		local alert = native.showAlert( "Android  is winner","congratulation", { "Ok" }, onComplete )
				end		
				flag = false
...
function DiceRotate( )
	...
	if flag and ai == true and dice~=nil  then
		flag = false
        ...
        transition.to( dice, {time=500, x = startX, y = startY} )
...
local function diceFunction( event )
	...
	firsttap=firsttap+1
	print("transition")
	if flag and tapenable and ai == false then
...
function scene:exitScene(event)
	group = self.view
	storyboard.removeScene( "snake1vsaiEXP" )
	storyboard.purgeScene( "snake1vsaiEXP" )
