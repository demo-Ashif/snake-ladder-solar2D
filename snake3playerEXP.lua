...
win=0
--intializing
	diceshot={0,0,0};--score of player to be saved
	playerCount = 0;--counting who can shot now
	playstart={0,0,0};--checking which player play the game
	playerwin={0,0,0};
	totalwin=0;
	winserial={0,0};
	firsttap=1
...
local function btnTap(event)
	...
	--storyboard.removeScene( "snake3playerEXP" )
...
local function btnTap1(event)
	p1.x = bg.x-1*bgw+3*bgsh;
	p1.y = display.contentHeight-60;
	p2.x = bg.x-2*bgw+20*bgsh;
	p2.y = display.contentHeight-60;
	p3.x = bg.x+1*bgw+0*bgsh;
	...
	transition.moveTo( p1, { x= bg.x-1*bgw+3*bgsh, y= display.contentHeight-60});
	transition.moveTo( p2, { x= bg.x-2*bgw+20*bgsh, y= display.contentHeight-60});
	transition.moveTo( p3, { x= bg.x+1*bgw+0*bgsh, y= display.contentHeight-60});

if animation1 ~= nil then
	animation1:removeSelf()
	animation1 = display.newSprite( mySheet1, sequenceData1 )
	animation1.x = bg.x-1*bgw+3*bgsh
...
local function gamelogic( )
		  ...
		  --print(i..playstart[i]);
				...
				if diceshot~=nil then
					...
					if  snake==1  or lad==1 then 
						...
						elseif i==3 then
							...
							if diceshot~=nil then 
					if diceshot[1]~=0 and diceshot[2]~=0 and diceshot[3]~=0  then
						print("fut");
				...
				if i==1 then
					...
					player.text="Player 2 turn!  ".. 100-diceshot[2].." to win"
						...
						if animation1 ~= nil then
							animation1:removeSelf()
							animation1 = display.newSprite( mySheet1, sequenceData1 )
							animation1.x = bg.x-2*bgw+20*bgsh
							animation1.y = display.contentHeight-30
							animation1:play()
						end
				else
						...
						if animation1 ~= nil then
							animation1:removeSelf()
							animation1 = display.newSprite( mySheet1, sequenceData1 )
							animation1.x = bg.x+1*bgw+0*bgsh;
							animation1.y = display.contentHeight-30
							animation1:play()
						end
			...
			if totalwin==2 then
				win=1
				print("why");
				--player.text="player"..i.."win";
				print("player"..i.."win");
				local alert = native.showAlert("Congratulation", "1st:Player  "..winserial[1].."    and   2nd:Player  "..winserial[2], { "Ok", }, onComplete )
				transition.moveTo( p1, { x= bg.x-1*bgw+3*bgsh, y= display.contentHeight-60});
				transition.moveTo( p2, { x= bg.x-2*bgw+20*bgsh, y= display.contentHeight-60});
				transition.moveTo( p3, { x= bg.x+1*bgw+0*bgsh, y= display.contentHeight-60});
				--starting new everything
